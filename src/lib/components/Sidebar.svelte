...
<aside class="flex flex-col grow overflow-hidden h-full">
  {#if user.activeUserTeam && user.activeUserTeam.team}
    ...
    {/if}
  {:else}
    <p class="text-white mt-4 text-opacity-60 text-center">
      Feeling like a taco with no toppings? Join a team or make one!
    </p>
  {/if}
</aside>
